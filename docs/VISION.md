...
| 31 | 2026-06-18 | Progressive model escalation on LLM failure | LLM Gateway escalates from small → medium → large model after 2+ consecutive failures per tier. Escalation is a safety net, not a feature — the default is always the cheapest viable model. Configurable per environment: disabled in dev (fail fast), 2-tier in e2e, 3-tier in prod. Supports 2 implementation options: Fixed Tiers, Provider-Cascade. |
